...
function _readSelectedData(_key){
  selectedData = fs.readFileSync(backupSelectFile);
...
function _writeSelectedData(_key, _data){
  ...
  fs.writeFileSync(backupSelectFile, JSON.stringify(selectedData))
}
...
function findDependenciesInSelection(_baseKeyInBackupFile, _baseKeyInMapFile, _targetKeyInMapFile) {
  let backupDataFromFile = JSON.parse(fs.readFileSync("backup.json"))[_baseKeyInBackupFile];
...
function findRelationShipsInDataServiceSelections() {
  let backupDataFromFile = JSON.parse(fs.readFileSync("backup.json")).dataservices;
...
function findRelatedConfigsUsedByFlows(){
  ...
  let backupFlowDataFromFile = JSON.parse(fs.readFileSync("backup.json")).flows;
...
function prepareFiles(){
  allconfigData = fs.readFileSync("backup.json");
  allconfigData = JSON.parse(allconfigData)
  let data = {};
  data["app"] = allconfigData["app"];
  data["groups"] = allconfigData["groups"];
  fs.writeFileSync("backup.tmp.json", JSON.stringify(data));
...
function _copyDataFromConfigToFinal(_key, _selectedKey){
  let data = fs.readFileSync("backup.tmp.json");
